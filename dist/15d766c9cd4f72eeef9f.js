    ...
    cytoscape('core', 'clipboard', function (opts) {
      ...
      function changeIds(jsons, pasteAtMouseLoc) {
        ...
        for (var j = 0; j < jsons.length; j++) {
          ...
          for (var k = 0; k < fields.length; k++) {
            var field = fields[k];
            if (json.data[field] && oldIdToNewId[json.data[field]]) json.data[field] = oldIdToNewId[json.data[field]];
          }
          if (json.position.x) {
      ...
      if (!scratchPad.isInitialized) {
        ...
        scratchPad.instance = {
          ...
          paste: function paste(_id, pasteAtMouseLoc) {
